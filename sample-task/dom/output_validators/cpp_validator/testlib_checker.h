 ...
 * problem format, modified from testlib.h. 
 * Most of the help messages from the original testlib.h will not work, please ignore them.
 *
 ...
 *
 * - team_message(std::string msg, ...):
 *        printf-style function for emitting an team message (a
 *        message that gets displayed to the team of the
...
const std::string FILENAME_team_MESSAGE = "teammessage.txt";
...
void team_message(const std::string msg, ...) {
    va_list pvar;
    va_start(pvar, msg);
    vreport_feedback(FILENAME_team_MESSAGE, msg, pvar);
}
...
{
    if (resultName == "" && feedbackdir == NULL)
    {
        textColor(color);
...
NORETURN void __testlib_help()
{
    ...
    std::fprintf(stderr, "    <input-file> <answer-file> feedback_dir < <output-file>\n\n");
...
void registerTestlibCmd(int argc, char* argv[])
{
    ...
    {
        quit(_fail, std::string("Program must be run with the following arguments: ") +
            std::string("<input-file> <answer-file> feedback_dir < <output-file>") +
            "\nUse \"--help\" to get help information");
    }

    if (!is_directory(argv[3])) {
        quit(_fail, std::string(argv[0]) + ": " + std::string(argv[3]) + " is not a directory");
    }

    feedbackdir = argv[3];
    {
        std::ostringstream fname;
        fname << feedbackdir << '/' << FILENAME_team_MESSAGE;
        truncate(fname.str().c_str());
    }
    {
        std::ostringstream fname;
        fname << feedbackdir << '/' << FILENAME_JUDGE_MESSAGE;
        truncate(fname.str().c_str());
    }
    {
        std::ostringstream fname;
        fname << feedbackdir << '/' << FILENAME_JUDGE_ERROR;
        truncate(fname.str().c_str());
    }


    if (argc == 5) // under construction
    {
        if (strcmp("-APPES", argv[4]) && strcmp("-appes", argv[4]))
        {
            quit(_fail, std::string("Program must be run with the following arguments: ") +
                        "<input-file> <answer-file> feedback_dir < <output-file>");
        }
        else
        {
            feedbackdir = argv[3];
